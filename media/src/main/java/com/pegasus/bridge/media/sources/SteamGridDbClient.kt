...
object SteamGridDbClient {
    ...
    fun search(term: String, apiKey: String): Result<List<SgdbGame>> {
        ...
        return HttpClient.get(url, headers(apiKey)).map { body ->
            val resp = JSONObject(body)
            if (!resp.optBoolean("success")) return@map emptyList()
            val data = resp.optJSONArray("data") ?: return@map emptyList()
    ...
    fun getGrids(gameId: Int, apiKey: String): Result<List<SgdbItem>> {
        ...
        return HttpClient.get(url, headers(apiKey)).map { parseItems(it) }
    }
