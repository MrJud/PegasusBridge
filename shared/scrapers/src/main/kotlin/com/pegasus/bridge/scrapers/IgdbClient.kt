...
object IgdbClient {
    ...
    internal var BASE = "https://api.igdb.com/v4"
    internal var TWITCH_TOKEN = "https://id.twitch.tv/oauth2/token"
    ...
    fun ensureToken(config: Config, clientId: String, clientSecret: String): Result<String> {
        ...
        return HttpClient.post(TWITCH_TOKEN, body, "application/x-www-form-urlencoded").mapCatching { resp ->
    ...
    fun search(term: String, clientId: String, token: String): Result<List<IgdbGame>> {
        ...
        return HttpClient.post("$BASE/games", query, "text/plain", postHeaders(clientId, token)).mapCatching { body ->
    ...
    fun getCovers(gameId: Int, clientId: String, token: String): Result<List<IgdbImage>> {
        ...
        return HttpClient.post("$BASE/covers", query, "text/plain", postHeaders(clientId, token)).mapCatching { parseImages(it, "t_1080p", "t_cover_big") }
    }
    ...
    fun getScreenshots(gameId: Int, clientId: String, token: String): Result<List<IgdbImage>> {
        ...
        return HttpClient.post("$BASE/screenshots", query, "text/plain", postHeaders(clientId, token)).mapCatching { parseImages(it, "t_1080p", "t_screenshot_big") }
    }

    // Mirrors getIGDBArtworks()
    fun getArtworks(gameId: Int, clientId: String, token: String): Result<List<IgdbImage>> {
        val query = "fields url,width,height; where game = $gameId; limit 15;"
        return HttpClient.post("$BASE/artworks", query, "text/plain", postHeaders(clientId, token)).mapCatching { parseImages(it, "t_1080p", "t_screenshot_big") }
    }
