...
object SteamStoreClient {
    ...
    internal var BASE = "https://store.steampowered.com/api"
    ...
    fun search(term: String): Result<List<SteamGame>> {
        ...
        return HttpClient.get(url).mapCatching { body ->
            val resp = JSONObject(body)
            val items = resp.optJSONArray("items") ?: return@mapCatching emptyList()
